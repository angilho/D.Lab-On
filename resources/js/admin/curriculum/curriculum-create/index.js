...
const AdminCurriculumCreate = ({ curriculum_category_id }) => {
	const history = useHistory();
	const [curriculumCategory, setCurriculumCategory] = useState(ctrl.getDefaultCurriculumCategory());
	const [showCurriculumCourseSearchModal, setShowCurriculumCourseSearchModal] = useState(false);

	const isEdit = curriculum_category_id ? true : false;

	useEffect(() => {
		if (isEdit) {
			ctrl.getCurriculumCategory(curriculum_category_id, result => {
				...
				setCurriculumCategory(result);
			});
		}
	}, []);

	const addCurriculumCourse = course => {
		let curriculumCourses = curriculumCategory.curriculum_courses;
		...
		setCurriculumCategory({ ...curriculumCategory, curriculum_courses: curriculumCourses });
	};

	const onClickCurriculumCourselUp = courseId => {
		let curriculumCourses = curriculumCategory.curriculum_courses.map(curriculumCourse => curriculumCourse);
	...
	const onClickCurriculumCourseDelete = courseId => {
		let curriculumCourses = curriculumCategory.curriculum_courses.filter(
	...
	return (
		<React.Fragment>
			<Row className="mt-3">
				<Col md={6}>
					...
					<FormControl
						...
						type="text"
						placeholder="커리큘럼 제목"
						value={curriculumCategory.title}
						onChange={event =>
							setCurriculumCategory({ ...curriculumCategory, title: event.currentTarget.value })
						}
					/>
				</Col>
			</Row>
			<Row className="mt-3">
				<Col md={6}>
					<FormLabel required>커리큘럼 소제목</FormLabel>
					<FormControl
						className="w-100"
						type="text"
						placeholder="커리큘럼 소제목"
						value={curriculumCategory.subtitle}
						onChange={event =>
							setCurriculumCategory({ ...curriculumCategory, subtitle: event.currentTarget.value })
						}
					/>
				</Col>
			</Row>
			<Row className="mt-3">
				<Col md={12}>
					...
					<TextAreaFormControl
						...
						as="textarea"
						rows={5}
						value={curriculumCategory.description}
						placeholder="내용을 입력해 주세요"
						onChange={event =>
							setCurriculumCategory({ ...curriculumCategory, description: event.currentTarget.value })
						}
					/>
				</Col>
			</Row>
			...
			{curriculumCategory.curriculum_courses.map((course, index) => {
				return (
						...
						<Col md={"auto"}>
							<CurriculumCourseContainer>
								...
								<Button
									...
									disabled={index === curriculumCategory.curriculum_courses.length - 1}
								>
									↓
								</Button>
			...
			<Row className="mt-5 mb-5 align-self-center">
				<Col md={2}>
					<Button
						...
						onClick={() => {
							if (isEdit) {
								ctrl.handleUpdate(curriculumCategory, () =>
									history.push({ pathname: "/admin/curriculums" })
								);
							} else {
								ctrl.handleCreate(curriculumCategory, () =>
									history.push({ pathname: "/admin/curriculums" })
								);
							}
						}}
...
export default AdminCurriculumCreate;
