...
export const getDetailCurriculumCategories = (query, callback) => {
	api.getDetailCurriculumCategories(query)
		.then(response => {
...
export const handleReorder = (curriculumCategories, callback) => {
	api.reorderDetailCurriculumCategory({ curriculum_categories: curriculumCategories })
		.then(response => {
	...
	if (confirm("선택한 커리큘럼을 삭제하시겠습니까?")) {
		await Promise.all(
			curriculumCategoryIds.map(curriculumCategoryId => {
				return api.deleteDetailCurriculumCategory(curriculumCategoryId);
			})
		)
			.then(resArr => {
